...
当环境不适合生存的时候，物种就会停止繁衍和生息，开始大规模迁徙。
...
- ✅️ `讨厌(不认可)` A 国治理, 不等于`喜欢(支持/鼓吹/认可)` B/C/D 国治理.
    - 哪里的月亮 `都不圆`, 但`一定不选`那个`最不圆的`. 🐶
    - 润, 只是过自己`可接受`的生活. 不润, 也一样. (自己接受就好)
- ✅️ 讨厌 A 国`统治者`(治理模式)  ≠ 讨厌 A 国`国民`. 反之, 也一样.
    - 区分 国家 vs 政党 vs 民族 vs 国民. (不要被带节奏)
- ✅️ A 国 `反贼(黑)` ≠ B/C/D 国 `支持者/信仰者(粉)`. (有人是, 但不是所有人)
    - 可能在 `A/B/C/D` 国, 仍然是 `反贼阵营`(名义上).
    - `在哪, 可能都是反贼`, `特立独行的猪🐷`.
- ✅️ `阶级(无产 vs 有产)认同` > `文化/宗教认同` >  `意识形态认同` > `组织认同` > `民族认同` >`国家认同`.
...
- ✅ 太穷. (贫贱不能移: 没有积蓄, 语言不过关, 无专业技能, 都算)
    - 没想到, 已经发展到了 `走线` 风靡. (竭泽而渔的后果)
    - 但, 我个人不建议这种方式. (如果年轻, 在国内当工具人, 攒几年钱💰, 再逃...)
    - 海外的骗子很多, 专骗同胞(他自然不拿你当自己人).
    - 海外最底层的工作, 并不美好. (骗子中介, 描绘的美好, 并不存在)
    - 那怎么办? 躺. 硬不起来, 等变天呗. (无解)
- ✅ 润不动的`老弱病残`.
